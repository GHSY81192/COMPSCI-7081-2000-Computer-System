...
static void visit_constructor(ast t) /*******/
{
    ...
    whileCounter = -1;
    ifCounter = -1;
    int varCounter = size_of_var_decs(get_subr_body_decs(subr_body));
    
    write_to_output("function " + className + "." + name + " " + to_string(varCounter) + "\n");
...
static void visit_if(ast t)
{
    ...
    int if_counter = ifCounter;
    ifCounter++;
    visit_expr(condition) ;
    write_to_output("if-goto IF_TRUE" + std::to_string(if_counter) + "\n");
    write_to_output("goto IF_FALSE" + std::to_string(if_counter) + "\n");
    write_to_output("label IF_TRUE" + std::to_string(if_counter) + "\n");
    visit_statements(if_true) ;
    write_to_output("label IF_FALSE" + std::to_string(if_counter) + "\n");
}
...
static void visit_if_else(ast t)
{
    ...
    write_to_output("label IF_TRUE" + std::to_string(if_counter) + "\n");
    visit_statements(if_true) ;
    write_to_output("goto IF_END" + std::to_string(if_counter) + "\n");
    write_to_output("label IF_FALSE" + std::to_string(if_counter) + "\n");
    visit_statements(if_false) ;
    write_to_output("label IF_END" + std::to_string(if_counter) + "\n");
}
...
static void visit_while(ast t)
{
    ...
    int while_counter = whileCounter;
    whileCounter++;
    write_to_output("label WHILE_EXP" + std::to_string(while_counter) + "\n");
    visit_expr(condition) ;
    write_to_output("not\n");
    write_to_output("if-goto WHILE_END" + std::to_string(while_counter) + "\n");
    visit_statements(body) ;

    write_to_output("goto WHILE_EXP" + std::to_string(while_counter) + "\n");
    write_to_output("label WHILE_END" + std::to_string(while_counter) + "\n");
}
...
static void visit_call_as_method(ast t) /*******/
{
    ...
    visit_subr_call(subr_call) ;
    write_to_output("call " + _class + "." + subr_name + " " + to_string(size_of_expr_list(expr_list)+1) + "\n");
}
