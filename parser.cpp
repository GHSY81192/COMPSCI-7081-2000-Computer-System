...
static string vtype;
static bool ifReturn = false;
static bool ifThis = false;
...
static ast parse_class()
{
    ...
    // add code here ...
    mustbe(tk_class);
    Token className = mustbe(tk_identifier);
    myclassname = token_spelling(className);
    mustbe(tk_lcb);
    ...
    ast ret = create_class(myclassname, statics, fields, subr_decs);
    pop_error_context() ;
    return ret ;
}
...
static ast parse_function()/******/
{
    ...
    mustbe(tk_function);
    ifThis = false;
    vtype = token_spelling(mustbe(tg_vtype));
    string name = token_spelling(mustbe(tk_identifier));
    mustbe(tk_lrb);
    ast params = parse_param_list();
    mustbe(tk_rrb);
...
static ast parse_statement()
{
    ...
    if(have(tk_return)){
        ifReturn = true;
        ret = create_statement(parse_return());
    }else if(have(tk_while)){
        ret = create_statement(parse_while());
    }else if(have(tk_if)){
        ret = create_statement(parse_if());
    }else if(have(tk_let)){
        ret = create_statement(parse_let());
    }else if(have(tk_do)){
        ret = create_statement(parse_do());
    }
    pop_error_context() ;
    return ret ;
}
...
static ast parse_let()//
{
    ...
    mustbe(tk_let);
    Token name = mustbe(tk_identifier);
    ast variable = lookup_variable_fatal(name);
    if(have(tk_lsb)){
        ast Variable = parse_index();
        mustbe(tk_assign);
        ast condition = parse_expr();
        mustbe(tk_semi);
        return create_let_array(variable, Variable, condition);
    }
    mustbe(tk_assign);
    ast condition = parse_expr();
    mustbe(tk_semi);
    ret = create_let(variable, condition);
    pop_error_context() ;
    return ret ;
}
...
static ast parse_void_this_call()/******/
{
    ...
    ast subr_call = parse_method_call();
    ast ret = create_call_as_method(myclassname, ast_this, subr_call);
    pop_error_context() ;
    return ret ;
}
...
static ast parse_return()
{
    ...
    if(have(tg_expression)){
        ...
        if(ifThis && !have(tk_this)){
            mustbe(tk_this);
        }
        ret = create_return_expr(parse_expr());
    }else{
        if(vtype != "void"){
...
static ast parse_expr()
{
    ...
    if(have(tg_infix_op)){
        ...
        ast op = create_infix_op(infix_op);
        ast rhs = parse_expr();
        ret = create_expr(lhs,op,create_term(rhs));
    }else{
        ret = create_expr(lhs);
    }
    pop_error_context() ;
...
// term ::= tk_integer | tk_string | tk_true | tk_false | tk_null | 
//(tk_lrb expr tk_rrb) | (tg_unary_op term) | var_term | this_term
...
static ast parse_term()
{
    ...
    if(have(tk_true)){
        ...
        Token tkinteger = mustbe(tk_integer);
        int intInteger = token_ivalue(tkinteger);
        ret = create_term(create_int(intInteger));
    }else if(have(tk_string)){
        string str = token_spelling(mustbe(tk_string));
        ret = create_term(create_string(str));
    }else if(have(tk_null)){
        mustbe(tk_null);
        ret = create_term(create_null());
    }else if(have(tk_lrb)){
        mustbe(tk_lrb);
        ...
        ast parseTerm = parse_term();
        ret = create_term(create_unary_op(op, parseTerm));
...
static ast parse_var_term()
{
    ...
    ast ret = create_empty() ;
    Token name = mustbe(tk_identifier);
    string nameString = token_spelling(name);
    if(have(tk_lsb)){
        ...
        ast subr_call = parse_fn_call();
        ret = create_call_as_function(nameString, subr_call);
    }else if(have(tk_stop)){
        ast object = lookup_variable_fatal(name);
        ast subr_call = parse_method_call();
        ret = create_call_as_method(get_var_type(object), object, subr_call);
    }else{
        ret = lookup_variable_fatal(name);
    }
    pop_error_context() ;
    return ret ;
}
...
static ast parse_this_term()
{
    ...
    if(have(tk_stop)){
        ast subr_call = parse_method_call();
        ret = create_call_as_method(myclassname, create_this(), subr_call);
    }else{
        ret = create_this();
    }
    pop_error_context() ;
