...
static ast optimise_class(ast t)
{
    ...
    ast statics_copy = optimise_class_var_decs(statics) ;
    ast fields_copy = optimise_class_var_decs(fields) ;
    ast subr_decs_copy = optimise_subr_decs(subr_decs) ;
    if(statics_copy == statics && fields_copy == fields && subr_decs_copy == subr_decs){
        return t;
    }
